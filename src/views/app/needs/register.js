...
const NeedRegister = (props) => {
  const { institutes } = useContext(AuthContext);
  const [need, setWorkshop] = useState({});
  const [needTopics, setWorkshopTopics] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const { type, needId } = useParams();

  const location = useLocation();
  console.log('teacher evaluation', needId);

  async function fetchData() {
    let data;
    setIsLoading(true);
    try {
      const response = await callApi('needs/', '', null);
      if (response.data && response.status === 200) {
        ...
        console.log('UPDATED Workshop DATA: ', need);
      } else {
  ...
  const getWorkshopTopics = () => {
    callApi('needs/topics/').then(async (response) => {
      if (response.data && response.status === 200) {
  ...
  const onSubmit = async (values) => {
    ...
    let params = { endPoint: 'needs/', method: 'POST' };

    if (needId) {
      params.endPoint = `needs/${needId}/`;
      params.method = 'PATCH';
    }
    await callApi(params.endPoint, params.method, values)
      .then((response) => {
        console.log('response in needs ', response.data);
        setIsNext(true);
      })
      .catch((error) => {
        console.log('Error in needs ', error);
      });
  };
          ...
          {!isNext ? (
            <Formik
              ...
              initialValues={{
                ...
                topic: needTopics.find((op) => op.value === need.topic) || '',
              }}
