  ...
  switch (type) {
    case 'success':
      NotificationManager.success(
        'نصاب په بریالیتوب سره دیلیت شو',
        'موفقیت',
        3000,
        null,
        null,
    ...
    case 'error':
      NotificationManager.error(
        'نصاب دیلیت نه شو بیا کوشش وکری',
        'خطا',
        9000,
        () => {
          alert('callback');
...
const CurriculumListBody = ({
  ...
  const handleClick = async (curriculumId) => {
    const instituteResponse = await callApi(
      `institute/department-subject/${curriculumId}/`,
      'DELETE',
      null
    );
